...
type AggregatedItem =
  | { type: "single"; reminder: ReminderRecord }
  | MedicationGroup;
...
const ReminderPage = () => {
  ...
  const getStatusLabel = (status: ReminderStatus) => {
    switch (status) {
      case "active":
        return t("reminders.active", "Đang chạy");
      case "paused":
        return t("reminders.paused", "Tạm dừng");
      case "expired":
        return t("reminders.expired", "Hết hạn");
      case "canceled":
        return t("reminders.canceled", "Đã hủy");
      default:
        return status;
    }
  };
  ...
  const [viewingGroup, setViewingGroup] = useState<MedicationGroup | null>(
    null,
  );
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(aggregatedItems.length / itemsPerPage),
  );
  ...
  const buildPayload = ():
    ...
    if (!formData.patientId) {
      showToast(
        t("reminders.patientRequired", "Vui lòng chọn bệnh nhân"),
        "error",
      );
      return null;
    }
    if (!formData.message.trim()) {
      showToast(
        t("reminders.messageRequired", "Vui lòng nhập nội dung"),
        "error",
      );
      return null;
    }
    ...
    for (const tVal of times) {
      if (
        Number.isNaN(tVal.hour) ||
        Number.isNaN(tVal.minute) ||
        tVal.hour < 0 ||
        tVal.hour > 23 ||
        tVal.minute < 0 ||
        tVal.minute > 59
        ...
        showToast(
          t(
            "reminders.invalidTime",
            "Giờ không hợp lệ (định dạng HH:mm, ví dụ: 08:00, 12:00)",
          ),
          "error",
        );
        return null;
  ...
  const loadReminders = async () => {
    try {
      ...
      const uniqueReminders = Array.from(
        new Map(reminders.map((item) => [item.id, item])).values(),
      );
      ...
      setViewingGroup((prev) => {
        if (!prev) return null;
        const latestReminders = uniqueReminders.filter(
          (r) =>
            r.patientId === prev.patientId &&
            r.prescriptionId === prev.prescriptionId,
        );
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (editingReminderId) {
        ...
        showToast(
          t("reminders.updateSuccess", "Cập nhật thành công"),
          "success",
        );
      } else {
        await createReminder(payload as ReminderBasePayload);
        showToast(
          t("reminders.createSuccess", "Tạo mới thành công"),
          "success",
        );
      }
      await loadReminders();
      ...
      console.error(error);
      showToast(
        error?.response?.data?.error || t("reminders.saveError", "Lỗi lưu"),
        "error",
      );
    } finally {
      setSaving(false);
  ...
  const handleQuickStatusUpdate = async (
    ...
    const actionLabel =
      nextStatus === "paused"
        ...
        : nextStatus === "active"
          ? "tiếp tục"
          : "hủy";
    if (!window.confirm(`Bạn có muốn ${actionLabel} nhắc nhở này không?`))
      return;
    ...
    try {
      ...
      if (editingReminderId === reminder.id)
        setFormData((c) => ({ ...c, status: nextStatus }));
    } catch (error: any) {
      ...
      <div className="w-full space-y-4 px-4 py-8 pb-24 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-800 dark:text-slate-100">
              {t("reminders.title", "Quản lý Nhắc nhở")}
            </h1>
            <p className="mt-2 text-gray-600 dark:text-slate-400">
              Thiết lập và theo dõi lịch nhắc nhở đo chỉ số, uống thuốc của bệnh
              nhân.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <select
                value={kindFilter}
                onChange={(e) =>
                  setKindFilter(e.target.value as ReminderKindFilter)
                }
                className="w-full rounded-xl border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-4 py-2.5 text-sm text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 outline-none"
              >
            ...
            <div>
              ...
              <select
                value={statusFilter}
                onChange={(e) =>
                  setStatusFilter(e.target.value as ReminderStatusFilter)
                }
                className="w-full rounded-xl border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-4 py-2.5 text-sm text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 outline-none"
              >
                {reminderStatusOptions.map((o) => (
                  <option key={o.value} value={o.value}>
                    {o.label}
                  </option>
                ))}
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-semibold text-gray-800 dark:text-slate-100">
            Nhắc nhở và đơn thuốc ({aggregatedItems.length} mục)
          </h2>
        </div>
            ...
            currentItems.map((item) => {
              if (item.type === "group") {
                ...
                return (
                  <div
                    key={item.id}
                    className="flex flex-col h-full rounded-xl border border-indigo-200 dark:border-indigo-900/50 bg-white dark:bg-slate-800 shadow-sm overflow-hidden"
                  >
                      ...
                      <div className="bg-indigo-50 dark:bg-indigo-900/30 px-5 py-4 flex justify-between items-start border-b border-indigo-100 dark:border-indigo-900/50">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            Mã BN: {item.patientCode || "N/A"}
                          </p>
                        </div>
                        <span
                          className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${getStatusClasses(item.status)}`}
                        >
                        ...
                        <div>
                          {item.reminders.length > 0 && (
                            <div className="mb-3 flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
                              ...
                              <span>
                                {formatDate(item.reminders[0].startDate)}
                              </span>
                              <span>→</span>
                              <span>
                                {formatDate(item.reminders[0].endDate)}
                              </span>
                            </div>
                          )}
                          ...
                          {(() => {
                            const rx = item.prescriptionId
                              ? prescriptionMap.get(item.prescriptionId)
                              : undefined;
                            if (!rx) return null;
                            const drugNames = [
                              ...new Set(rx.medications.map((m) => m.drugName)),
                            ];
                            if (drugNames.length === 0) return null;
                            return (
                              <ul className="mb-3 text-sm text-slate-700 dark:text-slate-300 space-y-0.5">
                                {drugNames.slice(0, 3).map((d, i) => (
                                  <li
                                    key={i}
                              ...
                              return (
                                <>
                                  {sortedTimes.slice(0, 4).map((t, i) => (
                                    <span
                                      key={i}
                                      className="bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs px-2 py-1 rounded-md font-medium border border-slate-200 dark:border-slate-600"
                                    >
                ...
                return (
                  <div
                    key={r.id}
                    className="flex flex-col h-full rounded-xl border border-sky-200 dark:border-sky-900/50 bg-white dark:bg-slate-800 shadow-sm overflow-hidden"
                  >
                    <div className="flex-1 flex flex-col">
                      <div className="bg-sky-50 dark:bg-sky-900/20 px-5 py-4 flex justify-between items-start border-b border-sky-100 dark:border-sky-900/50">
                        <div>
                          ...
                          <h3 className="font-bold text-slate-900 dark:text-slate-100">
                            {ptInfo?.name || r.patientId}
                          </h3>
                          <p className="text-xs text-slate-500">
                            Mã BN: {ptInfo?.code || "N/A"}
                          </p>
                        </div>
                        <span
                          className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${getStatusClasses(r.status)}`}
                        >
                          ...
                          <div className="grid grid-cols-2 gap-3 mb-4">
                            ...
                            <div className="bg-slate-50 dark:bg-slate-700/50 p-2.5 rounded-lg border border-slate-100 dark:border-slate-700">
                              ...
                              <p
                                ...
                                title={buildWeekdaySummary(r.daysOfWeek)}
                              >
                    ...
                    <div className="p-5 pt-0">
                      <div className="flex flex-wrap gap-2 mt-2 pt-4 border-t border-slate-100 dark:border-slate-700">
                        <button
                          onClick={() => applyReminderToForm(r)}
                          disabled={!canToggle || saving}
                          className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-slate-200 py-1.5 rounded-lg text-sm font-medium transition disabled:opacity-50"
                        >
                          Sửa
                        </button>
                        {r.status === "active" && (
                          <button
                            onClick={() =>
                              void handleQuickStatusUpdate(r, "paused")
                            }
                            disabled={saving}
                            className="flex-1 bg-amber-100 hover:bg-amber-200 text-amber-700 dark:bg-amber-900/40 dark:hover:bg-amber-800/60 dark:text-amber-400 py-1.5 rounded-lg text-sm font-medium transition disabled:opacity-50"
                          >
                            Dừng
                          </button>
                        )}
                        {r.status === "paused" && (
                          <button
                            onClick={() =>
                              void handleQuickStatusUpdate(r, "active")
                            }
                            disabled={saving}
                            className="flex-1 bg-emerald-100 hover:bg-emerald-200 text-emerald-700 dark:bg-emerald-900/40 dark:hover:bg-emerald-800/60 dark:text-emerald-400 py-1.5 rounded-lg text-sm font-medium transition disabled:opacity-50"
                          >
                            Tiếp tục
                          </button>
                        )}
                        {canToggle && (
                          <button
                            onClick={() =>
                              void handleQuickStatusUpdate(r, "canceled")
                            }
                            disabled={saving}
                            className="flex-1 bg-rose-50 hover:bg-rose-100 text-rose-600 dark:bg-rose-900/20 dark:hover:bg-rose-900/40 dark:text-rose-400 py-1.5 rounded-lg text-sm font-medium transition disabled:opacity-50"
                          >
                            Hủy
                          </button>
                        )}
        ...
        {totalPages > 1 && (
          <div className="mt-8 flex items-center justify-between border-t border-slate-200 dark:border-slate-700 pt-5">
            <div className="text-sm text-slate-500 dark:text-slate-400">
              ...
              {Math.min(endIndex, aggregatedItems.length)} của{" "}
              {aggregatedItems.length} mục
            </div>
            ...
            <div className="relative w-full max-w-3xl rounded-2xl bg-white dark:bg-slate-800 shadow-2xl flex flex-col max-h-[90vh]">
              <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 px-6 py-4">
                ...
                <button
                  ...
                  className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 transition"
                >
                  <FaTimes size={20} />
                </button>
              </div>
              ...
              <div className="p-6 overflow-y-auto flex-1">
                <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-900/50 rounded-xl p-4 mb-6">
                  <p className="text-sm text-indigo-800 dark:text-indigo-300 mb-2">
                    Đây là các lịch nhắc được tự động sinh ra từ đơn thuốc. Để
                    thay đổi giờ uống hoặc nội dung nhắc, vui lòng chỉnh sửa
                    trực tiếp trong Đơn Thuốc.
                  </p>
                  <Link
                ...
                {(() => {
                  ...
                  const translateMsg = (msg: string) => {
                    let r = msg.trim();
                    Object.entries(mealTimingMapModal).forEach(([en, vi]) => {
                      r = r.replace(new RegExp(en, "gi"), vi);
                    });
                    return r;
                  };
                  ...
                  viewingGroup.reminders.forEach((r) => {
                    ...
                    const drugs = (r.message || "")
                      .split(/;|\n/)
                      .map((s) => translateMsg(s))
                      .filter(Boolean);
                  ...
                  const reminderById = new Map(
                    viewingGroup.reminders.map((r) => [r.id, r]),
                  );
                  ...
                  return (
                    <div className="space-y-3">
                      {sortedSlots.map((slot) => {
                        ...
                        const slotStatus: ReminderStatus = statuses.includes(
                          "active",
                        )
                          ? "active"
                        ...
                        const uniqueReminderIds = Array.from(
                          new Set(slot.drugs.map((d) => d.reminderId)),
                        );
                          ...
                          >
                            <div className="flex items-center justify-between gap-3 mb-3">
                              <div className="flex items-center gap-2">
                                ...
                                <span
                                  className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold ${getStatusClasses(slotStatus)}`}
                                >
                                    ...
                                    <div key={rid} className="flex gap-1.5">
                                      {rObj.status === "active" && (
                                        <button
                                          ...
                                          disabled={saving}
                                          className="bg-amber-100 hover:bg-amber-200 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400 px-3 py-1.5 rounded-lg text-xs font-semibold transition disabled:opacity-50"
                                        >
                                          Tạm dừng
                                        </button>
                                      )}
                                      {rObj.status === "paused" && (
                                        <button
                                          ...
                                          disabled={saving}
                                          className="bg-emerald-100 hover:bg-emerald-200 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400 px-3 py-1.5 rounded-lg text-xs font-semibold transition disabled:opacity-50"
                                        >
                                          Kích hoạt
                                        </button>
                                      )}
                                      {(rObj.status === "active" ||
                                        ...
                                        <button
                                          ...
                                          disabled={saving}
                                          className="bg-rose-50 hover:bg-rose-100 text-rose-600 dark:bg-rose-900/20 dark:text-rose-400 px-3 py-1.5 rounded-lg text-xs font-semibold transition disabled:opacity-50"
                                        >
                                          Hủy
                                        </button>
                                      )}
              ...
              <div className="border-t border-slate-200 dark:border-slate-700 px-6 py-4 flex justify-end">
                <button
                  onClick={() => setViewingGroup(null)}
                  className="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-slate-200 rounded-lg text-sm font-semibold transition"
                >
                  Đóng
                </button>
              </div>
                  ...
                  <div>
                    ...
                    {formData.kind === "medication" && !editingReminderId && (
                      <p className="mt-1 text-xs text-amber-600 dark:text-amber-400">
                        Lưu ý: Đây là nhắc thuốc thủ công, không tự động đồng bộ
                        với đơn thuốc.
                      </p>
                    )}
                  </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {weekdayOptions.map((day) => {
                      const isSelected = formData.daysOfWeek.includes(
                        day.value,
                      );
                      return (
                        <button
