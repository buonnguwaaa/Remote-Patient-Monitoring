...
export default function PrescriptionDetailPage() {
  ...
  const [patientInfo, setPatientInfo] = useState<PatientDetailResponse | null>(
    null,
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch reminders
        const patientReminders = await getReminders({
          patientId: found.patientId,
          kind: "medication",
        });
        setReminders(
          patientReminders.filter((r) => r.prescriptionId === found.id),
        );

        // Fetch adherence (last 14 days)
        try {
          const adh = await getMedicationAdherence({
            patientId: found.patientId,
            days: 14,
          });
          setAdherence(adh);
  ...
  const handleStop = async () => {
    ...
    if (
      !window.confirm(
        "Bạn có chắc chắn muốn dừng đơn thuốc này? Lịch nhắc thuốc cũng sẽ bị dừng.",
      )
    )
      return;
    try {
  ...
  const adhRate =
    prescriptionAdherence.expected > 0
      ? Math.round(
          (prescriptionAdherence.taken / prescriptionAdherence.expected) * 100,
        )
      : 0;
  ...
  return (
    <div className="w-full px-4 py-8 pb-24 sm:px-6 lg:px-8">
      ...
      <button
        onClick={() => navigate("/prescriptions")}
        className="flex items-center text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 mb-6 transition font-medium"
      >
        <FaArrowLeft className="mr-2" /> Quay lại danh sách
      </button>
        ...
        <div className="bg-blue-50 dark:bg-slate-900/50 p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 dark:border-slate-700">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <span
                className={`px-3 py-1 text-xs font-bold uppercase tracking-wider rounded-full ${getStatusClasses(prescription.status)}`}
              >
          ...
          {prescription.status === "active" && (
            <div className="flex gap-3">
              <button
                onClick={() =>
                  navigate(`/prescriptions?patientId=${prescription.patientId}`)
                }
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="flex items-start gap-3 p-4 rounded-xl bg-slate-50 dark:bg-slate-700/30">
            ...
            <div>
              ...
              <p className="font-semibold text-slate-800 dark:text-slate-200">
                {prescription.daysOfWeek.length === 7
                  ? "Mỗi ngày"
                  : prescription.daysOfWeek.map((d) => WEEKDAYS[d]).join(", ")}
              </p>
            </div>
          </div>
          <div className="flex items-start gap-3 p-4 rounded-xl bg-slate-50 dark:bg-slate-700/30">
            ...
            <div>
              ...
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-xs text-slate-500">
                  ({prescriptionAdherence.taken}/
                  {prescriptionAdherence.expected} liều)
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2 mb-4">
            <FaCapsules className="text-blue-500" /> Chi tiết các loại thuốc (
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {med.schedule.map((dose, i) => (
                    <div
                      key={i}
                      className="bg-slate-50 dark:bg-slate-900/50 p-3 rounded-lg border border-slate-100 dark:border-slate-700 text-center"
                    >
                      <div className="text-sm font-bold text-slate-800 dark:text-slate-200 mb-1">
                        {dose.hour !== undefined
                          ? `${String(dose.hour).padStart(2, "0")}:${String(dose.minute).padStart(2, "0")}`
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-5 sticky top-6">
            {reminders.length === 0 ? (
              <p className="text-sm text-slate-500 text-center py-4">
                Đơn thuốc chưa sinh nhắc nhở nào.
              </p>
            ) : (
                    ...
                    prescription.medications.forEach((med) => {
                      med.schedule.forEach((dose) => {
                        ...
                        if (dh === t.hour && dm === t.minute) {
                          ...
                          drugs.push(
                            `${dose.pillCount} viên ${med.drugName}${meal ? ` (${meal})` : ""}`,
                          );
                        }
                      });
                    });
                    ...
                    <div key={r.id}>
                      ...
                      <div className="space-y-2">
                        {slotDrugs.map((slot, i) => (
                          <div
                            key={i}
                            className="flex items-start gap-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-600"
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              {slot.drugs.length === 0 ? (
                                <span className="text-xs text-slate-400 italic">
                                  Không có thuốc nào trong giờ này
                                </span>
                              ) : (
