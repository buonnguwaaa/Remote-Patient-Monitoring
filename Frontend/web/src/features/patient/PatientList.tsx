...
const PatientList = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 dark:bg-slate-900 md:p-8">
      <h1 className="mb-4 text-2xl font-bold text-gray-800 dark:text-slate-100 md:text-3xl">Danh sách bệnh nhân</h1>
          ...
          filteredPatients.map((patient, index) => (
            ...
            >
              <div className="mb-2 flex items-start justify-between gap-2">
                <div>
                  <p className="text-sm text-slate-500 dark:text-slate-400">STT {index + 1}</p>
                  <h3 className="text-base font-semibold text-slate-900 dark:text-slate-100">{patient.name}</h3>
                  <p className="text-sm text-slate-500 dark:text-slate-400">Mã hồ sơ: {patient.patientCode}</p>
                </div>
                {renderStatusBadge(patient.status)}
              </div>

              <div className="mb-3 text-sm text-slate-600 dark:text-slate-300">
                Ngày cập nhật: {patient.updatedAt || "-"}
              </div>
