...
export default function PrescriptionsScreen() {
  ...
  const fetchPrescriptions = useCallback(async ({ isRefresh = false } = {}) => {
    ...
    } catch { showToast("Không tải được danh sách đơn thuốc.", "error"); }
  ...
  const patientMap = useMemo(() => {
    const map = {};
    patients.forEach(p => map[p.patientId] = { name: p.patientName, code: p.patientPublicId });
    return map;
  }, [patients]);
  ...
  const handleSaveForm = async (data) => {
    ...
    try {
      ...
      if (res.ok || res.id || res._id || res.patientId) {
