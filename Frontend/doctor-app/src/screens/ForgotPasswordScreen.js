...
export default function ForgotPasswordScreen({ navigation }) {
  ...
  const handleStep1SendEmail = async () => {
    ...
    if (res.ok || res.status === 200) {
      showToast('Mã OTP 6 chữ số đã được gửi đến email của bạn.', 'success');
      setStep(2);
    } else {
      const serverErr = res.error?.error || res.error || 'Email không tồn tại trong hệ thống.';
      setErrorMsg(String(serverErr));
  ...
  const handleStep2VerifyOTP = async () => {
    ...
    if (res.ok || res.status === 200) {
      ...
      const serverErr = res.error?.error || res.error || 'Mã OTP không hợp lệ hoặc đã hết hạn.';
      setErrorMsg(String(serverErr));
      showToast(String(serverErr), 'error');
    }
  };
  ...
  const handleStep3ResetPassword = async () => {
    ...
    if (res.ok || res.status === 200) {
      ...
      const serverErr = res.error?.error || res.error || 'Không thể đặt lại mật khẩu. Vui lòng thử lại.';
      setErrorMsg(String(serverErr));
      showToast(String(serverErr), 'error');
    }
  };
  ...
  const handleResendOTP = async () => {
    ...
    setLoading(false);
    if (res.ok || res.status === 200) {
      showToast('Đã gửi lại mã OTP mới đến email của bạn.', 'success');
    } else {
      showToast(String(res.error?.error || res.error || 'Không thể gửi lại OTP.'), 'error');
    }
  };
  ...
  if (step === 4) {
    return (
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={[styles.container, { justifyContent: 'center', minHeight: '100%' }]} keyboardShouldPersistTaps="handled">
          <View style={{ alignItems: 'center', paddingHorizontal: 16 }}>
            <View style={{ backgroundColor: '#16A34A', width: 72, height: 72, borderRadius: 36, alignItems: 'center', justifyContent: 'center' }}>
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={[styles.container, { paddingTop: 48 }]} keyboardShouldPersistTaps="handled">
        {/* Top Back Navigation */}
        <TouchableOpacity
          onPress={() => {
            if (step > 1) {
          ...
          <View style={{ width: '100%' }}>
            ...
            <View style={[styles.form, { marginTop: 24 }]}>
              ...
              <TouchableOpacity onPress={() => navigation.navigate('Login')} style={{ alignItems: 'center', marginTop: 20 }}>
                <Text style={{ fontSize: 13, fontWeight: '600', color: '#2563EB' }}>Quay lại Đăng nhập</Text>
              </TouchableOpacity>
            </View>
          </View>
        )}
            ...
            <View style={styles.form}>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 20 }}>
                <TouchableOpacity onPress={handleResendOTP} disabled={loading}>
                  <Text style={{ fontSize: 13, fontWeight: '600', color: '#2563EB' }}>Gửi lại mã OTP</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => setStep(1)}>
                  <Text style={{ fontSize: 13, fontWeight: '600', color: '#2563EB' }}>Nhập lại email khác</Text>
                </TouchableOpacity>
              </View>
            </View>
          </View>
