...
export default function MeasurementInputScreen() {
  ...
  const displayPatients = (!normalizedSearch && !isListExpanded) 
    ? suggestedPatients.slice(0, 3) 
    : suggestedPatients;
          ...
          <View style={styles.card}>
            ...
            {assignmentsLoading ? (
              ...
              <View style={styles.assignmentChipWrap}>
                ...
                {!normalizedSearch && suggestedPatients.length > 3 && (
                  <TouchableOpacity 
                    style={styles.expandButton} 
                    onPress={() => setIsListExpanded(!isListExpanded)}
                  >
                    <Text style={styles.expandButtonText}>
                      {isListExpanded ? "Thu gọn" : `Xem thêm ${suggestedPatients.length - 3} bệnh nhân`}
                    </Text>
                    <Ionicons name={isListExpanded ? "chevron-up" : "chevron-down"} size={16} color="#6B7280" />
                  </TouchableOpacity>
                )}
              </View>
            ) : (
...
const styles = StyleSheet.create({
  ...
  assignmentEmptyText: { fontSize: 13, color: "#6B7280", fontStyle: "italic", textAlign: "center", marginVertical: 12 },
  expandButton: { flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: 8, marginTop: 4, gap: 4 },
  expandButtonText: { fontSize: 13, fontWeight: "600", color: "#6B7280" },
  patientCardWrap: { marginTop: 16 },
