...
\par Sau khi thống nhất định hướng phát triển, nhóm tiến hành lựa chọn các công cụ phục vụ cho quá trình quản lý mã nguồn, quản lý công việc, lưu trữ tài liệu, họp trực tuyến, thiết kế giao diện và vẽ sơ đồ hệ thống. Việc lựa chọn công cụ được cân nhắc dựa trên mức độ quen thuộc của các thành viên, khả năng hỗ trợ làm việc nhóm, tính phù hợp với quy mô đồ án và khả năng ứng dụng trong môi trường phát triển phần mềm thực tế. Các công cụ được nhóm sử dụng bao gồm GitHub, Jira Software, Google Drive, Google Meet, diagrams.net và Figma.

\subsubsection{Sử dụng GitHub để quản lý mã nguồn}
\par Nhóm sử dụng GitHub làm nơi quản lý tập trung toàn bộ mã nguồn của hệ thống. Do dự án bao gồm nhiều thành phần như backend, ứng dụng web dành cho bác sĩ, trang quản trị, ứng dụng di động và các file cấu hình triển khai, nhóm thống nhất tổ chức toàn bộ mã nguồn trong một monorepo chung trên GitHub. Các phần được phân chia theo thư mục như \texttt{Backend}, \texttt{Frontend/web}, \texttt{Frontend/admin}, \texttt{Frontend/mobile}, còn thư mục \texttt{.github/workflows} chứa các workflow CI/CD. Repository được tạo trên tài khoản của thành viên phụ trách và mời các thành viên khác cùng tham gia phát triển. Cách tổ chức này giúp nhóm dễ đồng bộ mã nguồn, theo dõi lịch sử thay đổi và phối hợp làm việc song song trên các phần khác nhau của hệ thống.
...
\begin{itemize}
    \item Thiết lập repository GitHub chung cho toàn bộ dự án, bố trí mã nguồn theo từng thành phần như backend, ứng dụng web, trang quản trị và ứng dụng di động. Repository cần có cấu trúc thư mục rõ ràng, hướng dẫn chạy dự án ở môi trường local và quy ước làm việc cơ bản để các thành viên có thể bắt đầu phát triển song song.
...
\par Bên cạnh trang quản trị, nhóm cũng tập trung hoàn thiện quy trình triển khai. Backend được triển khai trên AWS EC2 thông qua Docker Compose, Nginx được sử dụng làm reverse proxy, Redis và Temporal chạy như các service hỗ trợ, còn ứng dụng web bác sĩ và trang quản trị được build thành static files, lưu trữ trên Amazon S3 và phục vụ người dùng qua Cloudflare để thiết lập HTTPS. Việc triển khai hệ thống lên AWS kết hợp Cloudflare giúp nhóm có môi trường demo ổn định hơn so với việc chỉ chạy local, đồng thời giúp kiểm chứng khả năng vận hành thực tế của các thành phần client và server thông qua internet.
...
\begin{itemize}
    ...
    \item Cấu hình Cloudflare phía trước S3 để cung cấp HTTPS, ánh xạ tên miền và phục vụ frontend cho người dùng truy cập qua trình duyệt.
...
\par Quá trình triển khai lên AWS giúp nhóm phát hiện thêm nhiều vấn đề không xuất hiện khi chạy local, chẳng hạn như cấu hình CORS, biến môi trường production, security group, chứng chỉ HTTPS, đường dẫn API, reverse proxy cho WebSocket, cấu hình DNS trên Cloudflare và cách các service Docker giao tiếp với nhau trong cùng một network. Đối với frontend, nhóm cần cấu hình thêm Cloudflare để người dùng truy cập web qua HTTPS, đồng thời đảm bảo tên miền frontend trỏ đúng đến bucket S3 và các route của Single Page Application vẫn hoạt động khi người dùng refresh trang. Những vấn đề này làm tăng khối lượng công việc ở giai đoạn cuối, nhưng cũng giúp nhóm hiểu rõ hơn về quy trình đưa một hệ thống phần mềm từ môi trường phát triển lên môi trường triển khai thực tế. Đặc biệt, việc sử dụng Docker, GitHub Actions và Cloudflare giúp quy trình cập nhật backend và frontend trở nên ổn định hơn sau khi cấu hình ban đầu được hoàn tất.
...
\begin{longtable}{|p{3.2cm}|p{4.2cm}|p{6.5cm}|}
    ...
    Phụ trách chính xây dựng backend, thiết kế và cài đặt API, cấu hình hệ thống máy chủ, Docker, Nginx, CI/CD và triển khai lên AWS, Cloudflare. Ngoài ra, bạn Nhân còn phụ trách xử lý các vấn đề liên quan đến môi trường production, cấu hình biến môi trường, kết nối dịch vụ và kiểm tra hoạt động của backend sau khi deploy. \\
    \hline
...
\par Nhóm sử dụng GitHub làm công cụ quản lý mã nguồn và thống nhất không commit trực tiếp lên nhánh \texttt{master}. Khi bắt đầu một chức năng hoặc sửa một lỗi, thành viên phụ trách phải tạo một nhánh mới từ nhánh chính. Tên nhánh được đặt theo loại công việc để dễ nhận biết, ví dụ \texttt{feature/measurement-history}, \texttt{feature/alert-threshold}, \texttt{bugfix/login-refresh-token} hoặc \texttt{hotfix/deployment-config}. Việc đặt tên nhánh rõ ràng giúp các thành viên khác dễ hiểu mục đích của nhánh khi xem danh sách branch hoặc pull request.
...
\par Đối với các lỗi xảy ra trên môi trường AWS, nhóm cần kiểm tra nhiều lớp khác nhau như log của container, cấu hình Nginx, biến môi trường, security group, trạng thái Docker Compose, kết nối đến MongoDB, Redis hoặc Temporal. Đối với frontend, nhóm cũng cần kiểm tra thêm trạng thái DNS trên Cloudflare, cấu hình SSL/TLS, bản ghi CNAME trỏ đến S3 và các rule xử lý route của Single Page Application. Vì các lỗi triển khai thường khó tái hiện trên máy local, Nhân là người phụ trách chính việc kiểm tra hạ tầng, trong khi các thành viên khác hỗ trợ xác định lỗi từ phía client. Cách phối hợp này giúp nhóm phân biệt được lỗi do ứng dụng, lỗi do API, lỗi do cấu hình mạng, lỗi do Cloudflare hoặc lỗi do môi trường triển khai.
...
\par Ứng dụng web bác sĩ và trang quản trị được build thành static files và triển khai lên Amazon S3. So với việc chạy web trực tiếp trên EC2, cách triển khai này giúp tách biệt frontend và backend rõ ràng hơn. Frontend chỉ cần lưu trữ các file tĩnh, trong khi backend tập trung xử lý API. Nhóm sử dụng hai bucket S3 riêng biệt, một bucket cho ứng dụng web bác sĩ và một bucket cho trang quản trị. Sau khi GitHub Actions build ứng dụng, thư mục \texttt{dist} được đồng bộ lên bucket tương ứng bằng lệnh \texttt{aws s3 sync}.

\par Trên mỗi bucket, nhóm bật chế độ Static website hosting của S3 và cấu hình \texttt{index.html} làm trang mặc định. Đối với Single Page Application, nhóm cũng cấu hình trang lỗi \texttt{404} trỏ về \texttt{index.html} để khi người dùng truy cập trực tiếp một route nội bộ như \texttt{/patients} hoặc \texttt{/dashboard}, S3 vẫn trả về file gốc của ứng dụng và React Router có thể xử lý điều hướng phía client. Ngoài ra, nhóm cần đảm bảo biến môi trường \texttt{VITE\_API\_URL} được gán đúng URL API production trước khi build, vì giá trị này được nhúng vào bundle frontend tại thời điểm build.

\par Việc chỉ lưu trữ trên S3 chưa đủ để người dùng truy cập frontend qua HTTPS và tên miền tùy chỉnh. Vì vậy, sau khi đồng bộ static files lên S3, nhóm tiếp tục cấu hình Cloudflare làm lớp phía trước bucket để phục vụ giao diện web cho người dùng cuối.

\subsubsection{Cấu hình Cloudflare cho frontend}
\par Cloudflare được sử dụng để cung cấp HTTPS, quản lý DNS và chuyển tiếp request từ người dùng đến bucket S3 chứa static files. Trong mô hình triển khai của nhóm, S3 đóng vai trò nơi lưu trữ nội dung frontend, còn Cloudflare đóng vai trò điểm truy cập công khai mà người dùng sử dụng khi mở ứng dụng web bác sĩ hoặc trang quản trị. Cách tổ chức này giúp frontend có HTTPS mà không cần tự cấu hình chứng chỉ trực tiếp trên S3, đồng thời tách biệt rõ trách nhiệm giữa lưu trữ file và phân phối nội dung qua internet.

\par Quy trình cấu hình gồm các bước chính: thêm tên miền vào Cloudflare và cập nhật nameserver; tạo subdomain cho web bác sĩ và admin, dùng CNAME trỏ đến endpoint Static website hosting của bucket S3 tương ứng và bật proxy; cấu hình SSL/TLS để Cloudflare cung cấp HTTPS cho người dùng; cập nhật \texttt{FE\_WEB\_URL}, \texttt{FE\_ADMIN\_URL} và CORS trên backend; kiểm thử truy cập HTTPS, điều hướng SPA và gọi API production.

\par Trong quá trình cấu hình, nhóm thường gặp các lỗi như CNAME trỏ sai endpoint, \texttt{VITE\_API\_URL} build chưa đúng, backend chưa mở CORS cho domain mới hoặc SPA chưa fallback về \texttt{index.html} khi refresh route con. Các lỗi này được xử lý bằng cách kiểm tra lần lượt DNS, SSL/TLS, biến môi trường và cấu hình fallback trên S3.

\par Sau khi Cloudflare được cấu hình ổn định, quy trình cập nhật frontend trở nên đơn giản hơn. Khi có thay đổi ở mã nguồn web bác sĩ hoặc admin web, GitHub Actions build lại ứng dụng và đồng bộ file mới lên S3. Cloudflare tiếp tục phục vụ nội dung từ bucket mà không cần cấu hình lại DNS hoặc chứng chỉ, trừ khi nhóm thay đổi tên miền, bucket hoặc endpoint origin. Điều này giúp nhóm tách rõ quy trình phát hành frontend và backend: frontend được cập nhật bằng cách sync file lên S3, còn backend được cập nhật bằng cách pull Docker image mới trên EC2.
...
\par Cuối cùng, quá trình triển khai lên AWS và Cloudflare giúp nhóm hiểu rằng một ứng dụng chạy được ở local chưa chắc đã vận hành tốt ở môi trường production. Các vấn đề về biến môi trường, HTTPS, CORS, reverse proxy, WebSocket, Docker, security group, cấu hình DNS trên Cloudflare, SSL/TLS và secret management đều cần được chuẩn bị cẩn thận. Những kinh nghiệm này giúp nhóm không chỉ hoàn thành đồ án mà còn có thêm hiểu biết thực tế về quy trình phát triển, triển khai và vận hành một hệ thống phần mềm hoàn chỉnh.